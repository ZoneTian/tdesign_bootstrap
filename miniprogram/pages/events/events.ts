...
function parseDate(dateString: string): Date {
  ...
  // 将 ISO 8601 格式转换为 iOS 兼容格式
  // "2025-08-11T16:00:00.000+0000" -> "2025-08-11T16:00:00"
  let formattedDate = dateString;

  // 移除毫秒和时区信息，保留基本的 ISO 格式
  formattedDate = formattedDate.replace(/\.\d{3}[+-]\d{4}$/, '');
  formattedDate = formattedDate.replace(/\.\d{3}Z$/, '');
...
function getApplyStatus(applyStartTime: string, applyEndTime: string): applyStatus {
  const now = new Date();
  const startTime = parseDate(applyStartTime);
  const endTime = parseDate(applyEndTime);
...
function getActivityStatus(
  ...
  const now = new Date();
  const activityEndDate = parseDate(activityEndTime);
  const applyStartDate = parseDate(applyStartTime);
  const applyEndDate = parseDate(applyEndTime);
...
Page({
  ...
  async fetchActivityList(isLoadMore = false) {
    try {
      ...
      const result = await loadActivityList({
      ...
      if (result?.success && result?.data && Array.isArray(result?.data?.list)) {
        ...
        const newData = result?.data?.list.map(item => {
          ...
          return {
            ...
            condition: item?.applyCount && Number(item?.applyCount) > 0
              ? item?.applyCount + '人已上车'
              : '',
        ...
        if (isLoadMore) {
          this.setData({
            [`tabData.${currentTab}.activityList`]: [...(tabData.activityList || []), ...newData] as ActivityMpListVo[],
            [`tabData.${currentTab}.pageNum`]: tabData.pageNum + 1,
  ...
  onReachBottom() {
    ...
    if (!tabData.hasMore || tabData.loading) return;
